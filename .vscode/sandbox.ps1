New-Item -Path "$env:ProgramData\Microsoft\IntuneManagementExtension\Logs" -ItemType Directory -Force
Set-Location -Path "$PSScriptRoot"
Clear-Host
$Branch = (Get-ChildItem -Directory | Select-Object -First 1).Name

Write-Host "Testing branch $Branch"

if (Test-Path -Path .\$Branch\winget.ps1)
{
...
# Begin
Write-Host "Set working path $pwd\$Branch"
Set-Location .\$Branch\
Write-Host "Begin .\install.ps1"
. .\install.ps1
Start-Process explorer -ArgumentList "$env:ProgramData\Microsoft\IntuneManagementExtension\Logs"